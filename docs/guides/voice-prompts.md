...
- **Without an AEC, a prompt stays "live" for ~2 s after the audio drains.** That window lets STT discard echo of your own prompt instead of transcribing it as caller speech — and anything the caller says in it with it. `interrupt()` ends the window immediately if you need the next turn to start clean. With an AEC — in the pipeline, or the backend's own (`NATIVE_AEC`, e.g. `LocalAudioBackend(aec=...)`) — there is no such window: the prompt ends as soon as its audio has drained; a pipeline AEC keeps cancelling the room's echo tail for another 0.5 s.
