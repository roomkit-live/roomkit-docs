...
| `tail_padding_s` | `0.66` | Silence fed to a streaming transducer before its input ends. The model decodes its last frames only with audio behind them; without it the last words are cut ("Hello" → "Hell"). |
