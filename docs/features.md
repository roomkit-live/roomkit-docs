...
- Every tool call is auditable, the refused ones included: `ON_TOOL_CALL` reports the outcome of each call and states it with `is_error` instead of leaving it to be read out of the result text; a handler declines a call in its own words by raising `ToolRefusedError`; a call the model abandons (Gemini Live `tool_call_cancellation`) has its handler interrupted and is reported with `cancelled`
...
| `ON_AI_RESPONSE` | Async | A turn of intelligence completed — scoring, analytics, job tracking. `response_content` is the turn's transcript (the segments a tool call cut, separated by a blank line) and `segments` carries them one by one. `loop_end_reason` names which of the tool loop's rules ended the turn — `completed`, or `max_rounds` / `timeout` / `cancelled` / `force_stopped` / `truncated` / `empty_response` for a turn that was cut; infer that from `tool_calls_count` and a healthy multi-round answer reads as a cut-off. Fires for **any** channel of category `INTELLIGENCE`, an ACP coding agent included |
