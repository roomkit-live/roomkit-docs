...
| `ON_AI_RESPONSE` | Async | A turn of intelligence completed — scoring, analytics, job tracking. `response_content` is the turn's transcript (the segments a tool call cut, separated by a blank line) and `segments` carries them one by one. `loop_end_reason` names which of the tool loop's rules ended the turn — `completed`, or `max_rounds` / `timeout` / `cancelled` / `force_stopped` / `truncated` / `empty_response` for a turn that was cut; infer that from `tool_calls_count` and a healthy multi-round answer reads as a cut-off. `declared_tools` is the union, over every round of the turn, of the tools the provider received (name, description, `parameters` as declared), each with the reason Tool Search let it through (`always`, `pinned`, `sticky`, `revealed`); `BEFORE_AI_GENERATION` sees the first round only, so a tool `find_tools` revealed shows up here and nowhere else. Fires for **any** channel of category `INTELLIGENCE`, an ACP coding agent included (whose `declared_tools` stays empty: the toolset is the agent's) |
